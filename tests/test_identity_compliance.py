...
def test_weaver_identity_enforcement(mock_context):
    weaver = Weaver()
...
def test_context_library_identity_enforcement(mock_context):
    # register_context
...
def test_role_library_identity_enforcement(mock_context):
    # register_role
    with pytest.raises(ValueError, match="UserContext is required"):
...
def test_server_identity_enforcement(mock_context):
    component = PromptComponent(name="TestComp", type=ComponentType.CONTEXT, content="test")
